...
- [x] `STORAGE-015` 实现 `slot_a/slot_b`、活动槽指针、数据库关闭/重开和健康检查。验证：槽切换仪器测试。
- [x] `STORAGE-016` 实现 `pending_commit` 崩溃恢复和新槽失败回滚。验证：强杀恢复测试。
- [x] `STORAGE-017` 建立 5000 条错题基准数据生成器和查询计划断言。验证：首次列表、分页、筛选性能测试。

## 模块完成条件

- [x] 所有 Room、Keystore、图片和槽切换仪器测试通过。
- [x] `AC-CAP-002`、`AC-NFR-001`、`AC-NFR-003` 的存储侧条件通过。
