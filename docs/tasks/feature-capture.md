...
- [x] `CAPTURE-008` AI 配置缺失时保存 PendingOperation 并导航设置。验证：中断状态测试。
- [x] `CAPTURE-009` 配置成功后自动恢复同一草稿分析且只执行一次。验证：`AC-CAP-003`。
- [x] `CAPTURE-010` 实现确认页全部字段编辑和最多 5 个标签限制。验证：字段保存测试。
- [x] `CAPTURE-011` 保存时要求题集，成功后发出错题详情导航。验证：Fake Repository 测试。
- [x] `CAPTURE-012` 取消或失败后清理临时草稿，保存后只保留处理图。验证：`AC-CAP-002`。
- [x] `CAPTURE-013` 防止重复点击导致重复错题。验证：并发保存测试。
- [x] `CAPTURE-014` 添加拍照、相册到保存的完整 Compose/E2E 测试。验证：`AC-CAP-001`。
