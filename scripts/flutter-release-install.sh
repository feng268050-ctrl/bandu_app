#!/usr/bin/env bash
# Build and install the Flutter mobile client as a release APK.
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
FLUTTER_DIR="${FLUTTER_DIR:-${ROOT_DIR}/flutter_app}"
FLUTTER_PLATFORMS="${FLUTTER_PLATFORMS:-android}"
RELEASE_APK="${FLUTTER_RELEASE_APK:-${FLUTTER_DIR}/build/app/outputs/flutter-apk/app-release.apk}"
FLUTTER_BUILD_ONLY="${FLUTTER_BUILD_ONLY:-0}"
...
resolve_api_base_url() {
  ...
  die "set API_BASE_URL to a device-reachable URL, e.g. http://192.168.1.10:3000/api/mobile/v1"
}
...
if [[ "${FLUTTER_BUILD_ONLY}" != "1" ]]; then
  ...
  echo "INFO: release target ${SERIAL}" >&2
fi
...
[[ -f "${RELEASE_APK}" ]] || die "release APK not found: ${RELEASE_APK}"

if [[ "${FLUTTER_BUILD_ONLY}" == "1" ]]; then
  echo "INFO: built ${RELEASE_APK}" >&2
  exit 0
fi

"${ADB_BIN}" -s "${SERIAL}" install -r "${RELEASE_APK}"
echo "INFO: installed ${RELEASE_APK} on ${SERIAL}" >&2
