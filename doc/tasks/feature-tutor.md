...
- [x] `TUTOR-003` 实现新建无绑定会话和从错题创建绑定会话。验证：ViewModel 测试。
- [x] `TUTOR-004` 实现历史消息离线加载和 Markdown/LaTeX 展示。验证：无网络测试。
- [x] `TUTOR-005` 实现文本发送、流式增量和 250 ms UI 批量刷新。验证：Fake Stream 测试。
- [x] `TUTOR-006` 实现停止生成，保留已接收内容并允许继续追问。验证：`AC-AI-002`。
- [x] `TUTOR-007` 实现“分步讲解”快捷操作。验证：生成正确用户消息。
- [x] `TUTOR-008` 实现四档难度“生成类似练习”。验证：`AC-TUT-001`。
- [x] `TUTOR-009` 实现练习答案输入、AI 三态批改和反馈展示。验证：三态测试。
