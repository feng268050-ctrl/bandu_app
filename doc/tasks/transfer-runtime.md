...
- [ ] `RUNTIME-001` 创建 Android Library 模块和 Runtime public API。验证：`:transfer:runtime:compileDebugKotlin`。
...
- [ ] `RUNTIME-006` 集成首次 SRP 配对和双方身份确认。验证：错误码、过期码、限流测试。
- [ ] `RUNTIME-007` 实现 TrustedPeer DataStore、后续双向认证和解除配对。验证：身份变化视为新设备。
- [ ] `RUNTIME-008` 实现源端稳定快照、Manifest 和 `records.pbstream` 流式导出。验证：记录数和 hash 测试。
- [ ] `RUNTIME-009` 实现图片/缩略图 1 MiB 分块发送，内存保持常量。验证：5000 条数据内存基准。
- [ ] `RUNTIME-010` 实现目标端 resume bitmap 持久化和 24 小时过期。验证：进程重启续传测试。
- [ ] `RUNTIME-011` 实现非活动槽批量导入、外键/hash/图片/FTS 健康检查。验证：损坏包拒绝测试。
- [ ] `RUNTIME-012` 实现双方最终确认、活动槽切换、API Key 清除和失败回滚。验证：提交阶段强杀测试。
- [ ] `RUNTIME-013` 实现 `dataSync` Foreground Service、通知和可观察进度。验证：后台传输仪器测试。
- [ ] `RUNTIME-014` 实现取消、网络中断、空间不足和协议错误的清理/可恢复策略。验证：故障注入测试。
