...
- [x] `APP-011` 添加 Hilt 绑定，将 Repository、AI Provider 和 Transfer Runtime 注入 Domain 接口。验证：`:app:hiltJavaCompileDebug`。
- [x] `APP-012` 添加 App 导航端到端测试：全新启动进入首页，五个一级入口顺序正确。验证：`:app:connectedDebugAndroidTest`。

## 模块完成条件

- [x] `:app:assembleDebug`、导航测试和 Hilt 编译全部通过。
- [x] App 只负责装配和导航，不包含业务数据库或 Provider 实现。
