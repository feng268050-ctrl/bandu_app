...
- [ ] `STATS-001` 创建 Feature 模块和错题/练习双分区 Contract。验证：模块编译。
- [ ] `STATS-002` 展示错题总数、已掌握数和掌握率。验证：空数据和正常数据测试。
- [ ] `STATS-003` 展示错题学科分布和最近 6 个月趋势。验证：月份补零测试。
- [ ] `STATS-004` 展示练习总数、正确率和活跃天数。验证：统计值测试。
- [ ] `STATS-005` 展示练习学科、难度和月度趋势。验证：图表截图测试。
